    ...
    find "$input_dir" -type f | while read -r file; do
        ...
        if [[ ! "$target_file" =~ \.png$ ]]; then
            for find_color in "${!baseColors[@]}"; do
                ...
                #special sed for plank, instead of hex replace with rgb
                red=$(echo ${baseColors[$find_color]} | cut -c1-2)
                red=$((16#$red))
                green=$(echo ${baseColors[$find_color]} | cut -c3-4)
                green=$((16#$green))
                blue=$(echo ${baseColors[$find_color]} | cut -c5-6)
                blue=$((16#$blue))
                replace_rgb="$red;;$green;;$blue;;"
                sed -i "s/\*\*\*$find_color\*\*\*/$replace_rgb/g" "$target_file"
...
link_gtk4() {
	...
	echo -e "\nLinking '$theme_dir/gtk-4.0' to '${HOME}/.config/gtk-4.0'..."
...
install_theme() {
  ...
  cp -r $temp_dir/main/gnome-shell/pad-osd.css                                      $theme_dir/gnome-shell
	...
	cp $temp_dir/assets/gnome-shell/assets/*.svg  		                                $theme_dir/gnome-shell/assets
  ...
  # GTK2 Themes
	cp -r $temp_dir/main/gtk-2.0/gtkrc                				                        $theme_dir/gtk-2.0/gtkrc
	cp -r $temp_dir/assets/gtk-2.0/assets-common                                      $theme_dir/gtk-2.0/assets
	cp $temp_dir/assets/gtk-2.0/assets/*.png               		                        $theme_dir/gtk-2.0/assets
	cp $temp_dir/main/gtk-2.0/common/*.rc                                             $theme_dir/gtk-2.0
  ...
  cp -r $temp_dir/assets/gtk/thumbnails/thumbnail-Dark.png                          $theme_dir/gtk-3.0/thumbnail-Dark.png
	sassc $SASSC_OPT $temp_dir/main/gtk-3.0/gtk.scss                                  $theme_dir/gtk-3.0/gtk.css
  echo "@import url(\"gtk.css\");" >                                                $theme_dir/gtk-3.0/gtk-dark.css
  ...
  cp -r $temp_dir/assets/gtk/thumbnails/thumbnail-Dark.png                          $theme_dir/gtk-4.0/thumbnail-Dark.png
	sassc $SASSC_OPT $temp_dir/main/gtk-4.0/gtk.scss                                  $theme_dir/gtk-4.0/gtk.css
  echo "@import url(\"gtk.css\");" >                                                $theme_dir/gtk-4.0/gtk-dark.css

	# Cinnamon Themes
	cp -r $temp_dir/assets/cinnamon/common-assets                                     $theme_dir/cinnamon/assets
	cp -r $temp_dir/assets/cinnamon/assets/*.svg                                      $theme_dir/cinnamon/assets
	sassc $SASSC_OPT $temp_dir/main/cinnamon/cinnamon.scss                            $theme_dir/cinnamon/cinnamon.css
  ...
  for i in {1..3}; do cp $temp_dir/main/metacity-1/metacity-theme-3.xml             $theme_dir/metacity-1/metacity-theme-${i}.xml; done
	...
	#cp -r $temp_dir/assets/metacity-1/thumbnail.png 		                              $theme_dir/metacity-1/thumbnail.png

  # XFWM4 Themes
  mkdir -p                                                                          $theme_dir/xfwm4
	cp -r $temp_dir/assets/xfwm4/assets/*.png                                         $theme_dir/xfwm4
	cp -r $temp_dir/main/xfwm4/themerc                                                $theme_dir/xfwm4/themerc
...
if $blackness_flag; then
  blackness_color
fi

if $float_panel_flag; then
  float_panel
fi

if $outline_flag; then
  outline_style
fi
